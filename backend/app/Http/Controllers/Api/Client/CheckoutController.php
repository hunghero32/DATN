...
public function momoPayment(Request $request)
{
    ...
    $ipnUrl = "http://localhost:8000/api/client/momo-callback";
    $extraData = "";
...
public function momoCallback(Request $request)
{
    ...
    if ($resultCode == 0) {
        try {
            ...
            // If we couldn't extract booking_id from orderInfo, try to find it another way
            if (!$bookingId) {
                // You might store orderId to booking_id mapping when creating the payment
                // Or use any other logic to find the related booking
            }
            ...
            if ($bookingId) {
                ...
                if ($booking) {
                    ...
                    // You might want to update invoice status as well
                    $invoice = \App\Models\Invoice::where('booking_id', $bookingId)->first();
                    if ($invoice) {
