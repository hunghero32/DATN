/*
* Version: 1.3.0
* Template: xray Clinic And Patient Management Dashboard
...
(function () {
  ...
  const verticalProgress = (elem) => {
    const currentValue = elem.getAttribute('aria-valuenow')
    elem.style.height = '0%'
    elem.style.transition = 'height 2s'
    if (typeof Waypoint !== typeof undefined) {
      new Waypoint({
        element: elem,
        handler: function () {
          setTimeout(() => {
            elem.style.height = currentValue + '%'
          }, 100);
        },
        offset: 'bottom-in-view',
      })
  ...
  const customvVerticalBar = document.querySelectorAll('[data-toggle="progress-bar-vertical"]')
  Array.from(customvVerticalBar, (elem) => {
    verticalProgress(elem)
  })
  /*---------------noUiSlider-------------------*/
