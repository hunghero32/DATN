...
import { FaFileMedicalAlt, FaTimes, FaUserAlt, FaStethoscope, FaCalendarAlt, FaClock, FaDiagnoses, FaPrescriptionBottleAlt, FaStickyNote, FaDownload, FaPaperclip, FaEdit } from 'react-icons/fa';

// Helper function
const getDetailValue = (value, placeholder = 'Chưa cập nhật') => value || placeholder;

// Parse prescription data into an array
    ...
    <Modal show={true} onHide={onClose} centered size="lg" backdrop="static">
          ...
          <Row className="mb-3">
            <Col md={12}>
              ...
              <div style={{ padding: "10px", background: "#f8f9fa", borderRadius: "5px" }}>
                <Table striped bordered hover style={{ minHeight: "80px" }}>
                  <thead>
                    <tr>
                      <th>Tên Thuốc</th>
                      <th>Số lượng</th>
                  ...
                  <tbody>
                    {parsePrescription(result?.prescription).map((item, index) => (
                      <tr key={index}>
                        <td>{getDetailValue(item.medicine)}</td>
                        <td>{getDetailValue(item.quantity)}</td>
                        <td>{getDetailValue(item.note)}</td>
                      </tr>
                    ))}
                  </tbody>
                </Table>
      ...
      <Modal.Footer style={{ borderTop: "1px solid #edf2f9", padding: "20px", backgroundColor: "#f8f9fa", display: "flex", justifyContent: "space-between" }}>
        ...
        <Button
          variant="secondary"
          onClick={onClose}
          style={{ borderRadius: "8px", padding: "10px 20px", backgroundColor: "#6c757d", border: "none", display: "flex", alignItems: "center", gap: "8px" }}
        >
          <FaTimes /> Đóng
        </Button>
      </Modal.Footer>
